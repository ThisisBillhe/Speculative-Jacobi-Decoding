...
def process_token(args):
    i, next_token_logits, input_ids, logits_processor = args
...
class GenerationMixin:
    ...
    def _prepare_cache_for_generation(
        ...
        if generation_config.cache_implementation is not None:
            ...
            elif generation_config.cache_implementation == "zipcache": ## add here
                from ..cache_utils import ZipCache
                model_kwargs[cache_name] = ZipCache(generation_config.cache_config)
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(
            ...
            if num_image_start_tokens >= 1:
                ...
                image_token_num = 0
            if image_token_num > skiped_token_num - 1: ## the first two tokens are h_grids, w_grids
                # prepare model inputs
                all_ids = input_ids if all_ids is None else all_ids
                model_inputs = self.prepare_inputs_for_SJD_generation(all_ids, **model_kwargs)
                all_ids = model_inputs['input_ids']
                input_length = model_inputs['input_ids'].shape[1]

                if image_token_num == skiped_token_num and first_iter: ## deal with the first iters, special case
                ...
                # update input_ids with SJD
                # torch.cuda.synchronize()
                # start = time.perf_counter()

                scores = outputs.logits.clone().float().squeeze_(0)

                ## MultiModalLogitsProcessor
                mask_next_line = torch.zeros((scores.shape[0]), dtype=torch.bool, device=input_ids.device)
                mask_end_image = torch.zeros((scores.shape[0]), dtype=torch.bool, device=input_ids.device)
                indices = torch.arange(0, scores.shape[0], device=input_ids.device) + image_token_num
                mask_next_line[indices % (self.w_latent_dim + 1) == 0] = True
                mask_end_image[indices == ((self.w_latent_dim + 1) * self.h_latent_dim + 1)] = True
                scores[mask_next_line] = self.image_next_line_scores
                scores[~mask_next_line] += self.image_constrained_scores
                scores[mask_end_image] = self.image_end_scores

                ## top-K processor
                indices_to_remove = scores < torch.topk(scores, 1000)[0][..., -1, None]
                scores_topk_processed = scores.masked_fill(indices_to_remove, -float("Inf"))

                self.current_probs = nn.functional.softmax(scores_topk_processed, dim=-1)

                ## Sequential version
                # self.current_probs = []
                # for i in range(input_length):
                #     # next_token_logits = outputs.logits.clone()[:, i, :].float() ## deal with tokens one by one
                #     next_token_logits = scores_processed[:, i, :]
                #     # pre-process distribution
                #     next_token_scores = logits_processor(input_ids, next_token_logits) # dont worry about input_ids here
                #     # Store scores, attentions and hidden_states when required
                #     if return_dict_in_generate:
                #         if output_scores:
                #             scores += (next_token_scores,)
                ...
                #             )
                #     probs = nn.functional.softmax(next_token_scores, dim=-1)
                #     self.current_probs.append(probs.squeeze_(0))

                # torch.cuda.synchronize()
                # end = time.perf_counter()
                # print('Logits processing time: {:.2f}'.format((end-start)*1000), 'ms')
                
                # make decision on each token
                num_accepted_tokens = 0
                while True:
                    if self.accept_tokens_SJD(self.current_probs[num_accepted_tokens-1], num_accepted_tokens, all_ids[:,-self.sjd_window_size+num_accepted_tokens]):
                        next_tokens = all_ids[:,-self.sjd_window_size+num_accepted_tokens]
                        # update generated ids, model inputs, and length for next step
                        input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
                        if streamer is not None:
                            streamer.put(next_tokens.cpu())
                    ...
                    if num_accepted_tokens == input_length:
                        break

                # resample the token
                all_ids = input_ids
                for i in range(num_accepted_tokens, input_length):
                    # i == num_accepted_tokens or
                    if self.cached_probs is None or i > len(self.cached_probs) or torch.max(self.current_probs[i-1]) == 1 or torch.max(self.cached_probs[i-1]) == 1:
                        new_probs = self.current_probs[i-1]
                    else:
                        new_probs = torch.max(torch.tensor(0.0), self.current_probs[i-1] - self.cached_probs[i-1])

                        if torch.sum(new_probs) == 0:
                            new_probs = self.current_probs[i-1]
                        else:
                            # indices_to_remove = new_probs < torch.topk(new_probs, 1000)[0][..., -1, None]
                            # new_probs = new_probs.masked_fill(indices_to_remove, -float("Inf"))
                            new_probs = new_probs / torch.sum(new_probs)
                    # else:
                    #     new_probs = self.current_probs[i-1]

                    next_tokens = torch.multinomial(new_probs, num_samples=1)
                    all_ids = torch.cat([all_ids, next_tokens[:, None]], dim=-1)

                # save the prob, just overwrite the self.cached_probs
                self.cached_probs = self.current_probs[num_accepted_tokens:]
                
                # mainly deal with cache_position
                model_kwargs = self._update_model_kwargs_for_generation(
                    outputs,
                    model_kwargs,
                    is_encoder_decoder=self.config.is_encoder_decoder,
                    num_new_tokens=num_accepted_tokens
                )

                for layer_idx in range(len(model_kwargs["past_key_values"].key_cache)):
                    model_kwargs["past_key_values"].key_cache[layer_idx] = model_kwargs["past_key_values"].key_cache[layer_idx][:,:,:-self.sjd_window_size+num_accepted_tokens]
                    model_kwargs["past_key_values"].value_cache[layer_idx] = model_kwargs["past_key_values"].value_cache[layer_idx][:,:,:-self.sjd_window_size+num_accepted_tokens]

                unfinished_sequences = unfinished_sequences & ~stopping_criteria(input_ids, scores)
                this_peer_finished = unfinished_sequences.max() == 0
                
                if (input_ids[0] == 8196).sum() >= 2:
                    this_peer_finished = True
                #     input_ids = input_ids[:,:torch.where(input_ids[0] == torch.tensor([8710], device=input_ids.device))[1].item()+1]
                    
                cur_len += num_accepted_tokens
