...
from transformers.cache_utils import Cache, StaticCache, ZipCache
...
class ChameleonZipXAttention(ChameleonAttention):
    """
    Chameleon flash attention module. This module inherits from `ChameleonAttention` as the weights of the module stays
    untouched. The only required change would be on the forward pass where it needs to correctly call the public API of
    flash attention and deal with padding tokens in case the input contains any of them.
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Beware that with flash_attn<2.1, using q_seqlen != k_seqlen (except for the case q_seqlen == 1) produces a wrong mask (top-left).
        self._flash_attn_uses_top_left_mask = not is_flash_attn_greater_or_equal_2_10()
        self.compress_streaming_gap = 100
        self.compress_important_ratio = 0.95
    # Ignore copy
    def forward(
        self,
        hidden_states: torch.Tensor,
        ...
        **kwargs,
        ...
        if past_key_value is not None and not isinstance(past_key_value, ZipCache):
            # sin and cos are specific to RoPE models; position_ids needed for the static cache
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)

        elif past_key_value.get_seq_length(self.layer_idx) > 0 and isinstance(past_key_value, ZipCache):
            prev_keys, prev_values = past_key_value.fetch(self.layer_idx)
        ...
        if past_key_value is not None and isinstance(past_key_value, ZipCache):
            if q_len > 1: ## prefill phase, identify salient tokens
                if q_len > 128:
                    probe_ids = torch.cat([torch.arange(q_len-64, q_len), \
                        torch.randint(0, q_len-64, (64, ))]) ## use rand + recent tokens
                else:
                    probe_ids = torch.arange(0, q_len)
                probe_query_states = query_states[:,:,probe_ids,:]

                if attention_mask is None:
                    attention_mask = self.create_causal_mask_2d(q_len, dtype=query_states.dtype, device=query_states.device)
                ...
                token_attn_sum = torch.sum(attn_weights, dim=[1,2])
                token_attn_mean = token_attn_sum / torch.flip(torch.arange(1,token_attn_sum.shape[-1]+1, device=token_attn_sum.device), dims=(0,)) ## in case token_attn_sum = torch.sum(attn_weights, dim=[1,2])

                ## code to analyze attention distribution
                tmp = token_attn_sum.sum(0).float() ## in case token_attn_sum = torch.sum(attn_weights, dim=[1,2])
                total_sum = tmp.sum()
                sorted_tensor = torch.sort(tmp, descending=True).values
                cumulative_sum = sorted_tensor.cumsum(0)
                threshold = 0.975 * total_sum
                valid_index = torch.where(cumulative_sum >= threshold)[0][0] + 1
                valid_index = valid_index.item()

                # print('layer index: ', self.layer_idx, ' number of elements to achieves 0.975 attn sum:', valid_index)
        
                self.important_ids = token_attn_mean.topk(valid_index, dim=-1, largest=True).indices
            else:
                self.counter += 1 ## count how many decoding steps now

            if self.counter != 0 and self.counter % self.compress_streaming_gap == 0: ## need compress every streaming_gap tokens
                attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
                attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
                token_attn_sum = torch.sum(attn_weights, dim=[1,2])
                token_attn_sum = token_attn_sum[:,self.prompt_length:]
                new_important_ids = token_attn_sum.topk(int(self.compress_important_ratio*self.compress_streaming_gap), dim=-1, largest=True).indices + self.prompt_length
                self.important_ids = torch.cat([self.important_ids, new_important_ids], dim=-1)
                self.prompt_length += self.compress_streaming_gap

            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position, "salient_ids": self.important_ids}
    ...
    def create_causal_mask_2d(self, q_len, dtype, device):
        mask = torch.full((q_len, q_len), torch.finfo(dtype).min, device=device)
...
class ChameleonDecoderLayer(nn.Module):
    def __init__(self, config: ChameleonConfig, layer_idx: int):
        ...
        # self.self_attn = ChameleonZipXAttention(config=config, layer_idx=layer_idx)
...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def forward(
        ...
        if cache_position[0] != 0 and input_ids.shape[1] != 1: ## using SJD
            causal_mask = self._update_causal_mask_SJD(
    ...
    def _update_causal_mask_SJD(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            causal_mask = torch.full((sequence_length, target_length), fill_value=min_dtype, dtype=dtype, device=device)
            if sequence_length != 1:
                offset = target_length - sequence_length
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        ## Hyper-parameters for SJD
        self.sjd_window_size = 8
        self.cached_probs = None
        self.w_latent_dim = 48
        self.h_latent_dim = 48
        self.image_next_line_scores = torch.full((65536,), -math.inf, dtype=torch.float32, device='cuda')
        self.image_next_line_scores[8803] = 0
        self.image_end_scores = torch.full((65536,), -math.inf, dtype=torch.float32, device='cuda')
        self.image_end_scores[8196] = 0
        self.image_constrained_scores = torch.full((65536,), -math.inf, dtype=torch.float32, device='cuda')
        self.image_constrained_scores[4:8196] = 0.0
    ...
    def prepare_inputs_for_SJD_generation(
        self,
        input_ids,
        pixel_values=None,
        past_key_values=None,
        ...
        **kwargs,
        ...
        image_start_token_id_index = torch.where(input_ids[0] == 8197)[0]
        image_token_num = input_ids.shape[1] - image_start_token_id_index - 1 - 2 # two w&h shape tokens
        pad_length = self.sjd_window_size - input_ids[:, cache_position:].shape[1]
        # if image_token_num > 49 and input_ids[0,-1] == 8803:
        #     pad_value = input_ids[:, -49].item()
        # elif image_token_num > 49:
        #     pad_value = input_ids[:, -1].item() if random.random() < 0.5 else input_ids[:, -49].item()
        # elif image_token_num<= 49 and input_ids[0,-1] == 8803:
        #     pad_value = input_ids[:, -2].item()
        # else:
        #     pad_value = input_ids[:, -1].item()
        if image_token_num<= 49 and input_ids[0,-1] == 8803:
            pad_value = input_ids[:, -2].item()
            input_ids = input_ids[:, cache_position:]
            input_ids = F.pad(input_ids, (0, pad_length), mode='constant', value=pad_value)
        elif image_token_num<= 49:
            pad_value = input_ids[:, -1].item()
            input_ids = input_ids[:, cache_position:]
            input_ids = F.pad(input_ids, (0, pad_length), mode='constant', value=pad_value)
        elif image_token_num > 49 and input_ids[0,-1] == 8803:
            pad_values = input_ids[:,-49:-49+pad_length]
            input_ids = input_ids[:, cache_position:]
            input_ids = torch.cat([input_ids, pad_values], dim=-1)
        else:
        ...
        attention_mask = torch.ones((1, cache_position + self.sjd_window_size), device=attention_mask.device, dtype=attention_mask.dtype)
        if attention_mask is not None and position_ids is None:
            # create position_ids on the fly for batch generation
            position_ids = attention_mask.long().cumsum(-1) - 1
            position_ids.masked_fill_(attention_mask == 0, 1)
    ...
    def accept_tokens_SJD(self, probs, num_new_tokens, token_id):
        if num_new_tokens == 0:
            # always accept the first token
            return True
        elif self.cached_probs is None or num_new_tokens >= len(self.cached_probs):
            next_tokens = torch.multinomial(probs, num_samples=1)
            if next_tokens == token_id:
                return True
            else:
                return False
        else:
            prev_probs = self.cached_probs[num_new_tokens-1]
            accept_prob = min(1.0, (probs[token_id] / prev_probs[token_id]).item())
            # if torch.multinomial(torch.tensor([1-accept_prob, accept_prob]), num_samples=1):
